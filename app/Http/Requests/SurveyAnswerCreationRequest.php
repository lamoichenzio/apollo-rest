...
class SurveyAnswerCreationRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'answers.*.answer' => [
                'string',
                'required_without_all:answers.*.answers,answers.*.answerPair,answers.*.answersPair'],
            'answers.*.answers' => 'array|required_without_all:answers.*.answer,answers.*.answerPair,answers.*.answersPair',
            'answers.*.answers.*' => 'sometimes|required|string',
            'answers.*.answerPair' => 'array|required_without_all:answers.*.answers,answers.*.answer,answers.*.answersPair',
            'answers.*.answerPair.*.element' => 'required|exists:matrix_question_elements,id',
            'answers.*.answerPair.*.answer' => 'required|string',
            'answers.*.answersPair' => 'array|required_without_all:answers.*.answers,answers.*.answerPair,answers.*.answer',
